...
| Exact URL used | `https://api.stlouisfed.org/fred/series/observations?series_id=GFDEBTN&observation_start=1966-01-01&file_type=json` |
...
Collected as the work order's named alternative source. It serves two purposes
the daily Treasury file cannot: it reaches back before 1993, covering the
...
These three files were collected as a one-off and are **not** wired into
`main.py` or the daily `update.yml` workflow, so they will not refresh on their
own. Re-running the URLs above reproduces them.
